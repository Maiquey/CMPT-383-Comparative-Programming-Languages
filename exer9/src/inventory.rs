...
pub fn total_value<'a>(items: impl IntoIterator<Item = &'a InventoryItem>) -> f64 {
    items.into_iter().fold(0.0, |acc, item| acc + (item.count as f64 * item.cost))
}
...
pub fn explode<'a>(items: impl IntoIterator<Item = &'a InventoryItem>) -> Vec<InventoryItem> {
    items.into_iter().flat_map(|item| (0..item.count).map(|_| InventoryItem::new(1, item.cost, item.description.clone()))).collect()
}
